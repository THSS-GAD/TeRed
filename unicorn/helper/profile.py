...
class BestClusterGroup:
    ...
    def __init__(
        self, best_num_clusters=-1, best_cluster_labels=None, best_medoids=None
    ):
    ...
    def optimize(
        ...
        max_cluster_num = min(
            arrs.shape[0], max_cluster_num
        )  # max_cluster_num should never be bigger than the total number of arrays
        for num_clusters in range(2, max_cluster_num):
            for trial in range(num_trials):
                # We use _k_medoids_spawn_once from medoids.py for computation.
                _, medoids = _k_medoids_spawn_once(
                    points=range(arrs.shape[0]),
                    k=num_clusters,
                    distance=distance,
                ...
                actual_num_cluster = len(
                    medoids
                )  # @num_clusters is the maximum possible cluster. The actually number (@actual_num_cluster) may be smaller.
                for medoid_idx, medoid in enumerate(medoids):
                    for (
                        element
                    ) in (
                        medoid.elements
                    ):  # medoid.elements are all the array indices that belong to a medoid's cluster.
                        cluster_labels[element] = medoid_idx
                cluster_labels = np.array(cluster_labels)

                if (
                    actual_num_cluster == 1
                ):  # If the ideal number of cluster is 1, we cannot use silhouette_score function, which requires at least two clusters.
                    silhouette_coef = -1  # We prefer more than 1 cluster.
                else:
                    silhouette_coef = silhouette_score(
                        arrs, cluster_labels, metric=method
                    )
                # We decide if this run is the best value we have seen.
                if silhouette_coef > best_silhouette_coef or (
                    silhouette_coef == best_silhouette_coef
                    and actual_num_cluster > self.best_num_clusters
                ):  # We prefer larger cluster number.
...
class Model:
    ...
    def construct(self, arrs, dists, best_cluster_group):
        ...
        for cluster_idx in range(best_num_clusters):
            cluster_centers[cluster_idx] = best_medoids[
                cluster_idx
            ].kernel  # kernel is the index of the medoid sketch
            self.medoids[cluster_idx] = arrs[
                cluster_centers[cluster_idx]
            ]  # the actually sketch array of the medoid
            self.members[cluster_idx] = best_medoids[
                cluster_idx
            ].elements  # elements is a list of indices of all the members in the cluster
            # cluster_dists contains all the distances between the kernel (medoid) and each member array
            cluster_dists = [
                dists[cluster_centers[cluster_idx]][elem]
                for elem in self.members[cluster_idx]
                if elem != cluster_centers[cluster_idx]
            ]
            if (
                len(cluster_dists) == 0
            ):  # This cluster has only one member (this is usually not good)
        ...
        for elem_idx in range(
            arrs.shape[0]
        ):  # We go through every array element to construct the evolution
            for cluster_idx in range(best_num_clusters):
                if (
                    elem_idx in self.members[cluster_idx]
                    or elem_idx == cluster_centers[cluster_idx]
                ):
...
def test_single_graph(arrs, models, metric, num_stds, debug_info=None):
    ...
    abnormal_point = (
        []
    )  # @abnormal_point is valid only if eventually @abnormal is True.
    ...
    num_fitted_model = (
        0  # The total number of models that can be fitted by the test graph.
    )
    # Additional logic for debugging only
    if isinstance(
        debug_info, dict
    ):  # debug_info is either None (no debugging) or a dictionary
        ...
        fitted_models = (
            list()
        )  # fitted_models records all the models that can be fitted
    ...
    for model in models:
        ...
        if metric == "mean":
            current_threshold = (
                model.get_mean_thresholds()[current_cluster_idx]
                + num_stds * model.get_stds()[current_cluster_idx]
            )
        elif metric == "max":
            current_threshold = (
                model.get_max_thresholds()[current_cluster_idx]
                + num_stds * model.get_stds()[current_cluster_idx]
            )

        for arr_id, sketch in enumerate(arrs):
            distance_from_medoid = hamming(
                sketch, current_medoid
            )  # Compute the hamming distance between the current medoid and the current test sketch.
            if distance_from_medoid > current_threshold:
                # Check maybe the evolution has evolved to the next cluster if it exsits.
                if (
                    current_evolution_idx < len(model.get_evolution()) - 1
                ):  # If there is a next cluster in evolution.
                    current_evolution_idx = (
                        current_evolution_idx + 1
                    )  # Move on to the next cluster and see if it fits.
                    current_cluster_idx = model.get_evolution()[current_evolution_idx]
                    current_medoid = model.get_medoids()[current_cluster_idx]
                    if metric == "mean":
                        current_threshold = (
                    ...
                    if (
                        distance_from_medoid > current_threshold
                    ):  # If it still does not fit, we consider it abnormal
                        check_next_model = (
                            True  # We know this graph does not fit into this model,
                        )
                        # but it may fit into a different model.
                        abnormal_point.append(
                            arr_id
        ...
        for model in models:
            ...
            for arr_id, sketch in enumerate(arrs):
                ...
                for cluster in model.get_evolution():
                    ...
                    if metric == "mean":
                        threshold = (
                            model.get_mean_thresholds()[cluster]
                            + num_stds * model.get_stds()[cluster]
                        )
                    elif metric == "max":
                        threshold = (
                            model.get_max_thresholds()[cluster]
                            + num_stds * model.get_stds()[cluster]
                        )
                    sketch_info[arr_id].append(
                        (cluster, distance_from_medoid - threshold)
                    )
