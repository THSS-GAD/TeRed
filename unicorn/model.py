...
def model_graphs(
    train_files, model_file, max_cluster_num=6, num_trials=20, max_iterations=1000
):
    ...
    if model_file:
        ...
        print(
            "\33[5;30;42m[INFO]\033[0m Model is not saved, use --save-model to save the model"
        )
    for train_file in train_files:
        with open(train_file, "r") as f:
            ...
            try:
                ...
                print(
                    "\33[101m[ERROR]\033[0m Exception occurred in modeling from file {}: {}".format(
                        train_file, e
                    )
                )
            ...
            print(
                "\x1b[6;30;42m[SUCCESS]\x1b[0m Model from {} is done...".format(
                    train_file
                )
            )
            ...
            if model_file:
                print(
                    "\x1b[6;30;42m[STATUS]\x1b[0m Saving the model {} to {}...".format(
    ...
    for test_file in test_files:
        with open(test_file, "r") as f:
            ...
            if isinstance(DEBUG_INFO, dict):
                test_info = dict()
            sketches = load_sketches(f)
            abnormal, max_abnormal_point, num_fitted_model = test_single_graph(
                sketches, models, metric, num_stds, test_info
            )
            if isinstance(DEBUG_INFO, dict):
        ...
        if not abnormal:  # The graph is considered normal
            printout += "{} is NORMAL fitting {}/{} models\n".format(
                test_file, num_fitted_model, len(models)
            )
            if (
                "attack" not in test_file
            ):  # NOTE: file name should include "attack" to indicate the oracle
                tn = tn + 1
            else:
                fn = fn + 1
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--model-path", help="file path to save the model", default="model.txt"
    )
    parser.add_argument(
        "-c",
        "--cross-validation",
        help="number of cross validation we perform (use 0 to turn off cross validation)",
        type=int,
        default=5,
    )
    parser.add_argument(
        "-v", "--verbose", help="produce debugging information", action="store_true"
    )
    parser.add_argument(
        "-C", "--clusters", help="max num of the final clusters", type=int, default=6
    )
    ...
    if args.verbose:
        print(
            "\33[5;30;42m[INFO]\033[0m Debugging information will be collected. You can access it programmatically"
        )
        DEBUG_INFO = dict()
    ...
    if (
        not args.num_stds
    ):  # If this argument is not given, we explore different possible configurations.
    ...
    models = model_graphs(train_files, model_save_path, max_cluster_num=args.clusters)

    # Perform K-fold cross validation, unless turned off
    if args.cross_validation == 0:
        print(
            "\33[5;30;42m[INFO]\033[0m No cross validation specified, use --cross-validation"
        )
        ...
        for tm in metric_config:
            for ns in std_config:
                precision, recall, accuracy, f_measure, printout = test_graphs(
                    test_files, submodels, tm, ns
                )
                print("Metric: {}\tSTD: {}".format(tm, ns))
                print(
                    "Accuracy: {}\tPrecision: {}\tRecall: {}\tF-1: {}".format(
        ...
        for train_idx, validate_idx in kf.split(train_files):
            ...
            print(
                "\x1b[6;30;42m[STATUS] Test {}/{}\x1b[0m:".format(
